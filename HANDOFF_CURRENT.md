...
更新日: 2026-09-07 (Asia/Tokyo)

## 1. 対象・コンセプト・現在工程

- 対象: 新規iPhoneアプリ「星を聴く / Hear the Stars」。
- 現在地・日時・恒星座標とiPhoneの真北方位/姿勢を端末内で照合し、選んだ星へ近づくほど音と振動が速く・明瞭になる体験。
- 現在工程: Build 2で方角準備・調整・完了の表示と、結果からの再探索/星選択を追加。日英・構造監査済み、クラウドテスト/ビルドを準備中。
...
- 方位精度good/fair・新鮮な姿勢・現在地が揃った時だけ「方角の準備ができました」を表示し案内する。不良方位・移動・センサー停止で解除し、理由と対処を示す。
...
- Build: `2`（準備状態・戻り導線の改善。前回IPAはBuild 1）
...
- VoiceOver、音、触覚、Core Motion、実機表示は未確認。Build 1のXcodeコンパイルは成功済み。
...
1. Build 2をCodemagicでテスト/ビルドし、更新IPAを取得する。
2. 実機で準備→調整→案内可能、状態悪化→解除、設定から復帰、結果→再探索/星選択、星座→結果への戻りを確認する。
...
- 正本Git状態: `main`、既存 `01e363d` を基点にBuild 2改善を適用予定。作業コピーはcwd配下の `hear-stars`。

...
| F-06 | 有効方位精度は good（8°以下）、fair（8°超10°以下）、needsCalibration（10°超25°以下）、unavailable（無効値または25°超）に分類する。good/fair は発見可、needsCalibration は粗い案内だけ、unavailable・データ陳腐化・移動中は全方向案内と発見を停止する。磁北値へ無表示で切り替えない。 |
...
- Phase 1 の有効方位精度は `good: 0°以上8°以下`、`fair: 8°超10°以下`、`needsCalibration: 10°超25°以下`、`unavailable: 有効値なし、負値または25°超` とする。
- good/fair は発見判定に参加できる。needsCalibration は補正を促しつつ粗い方向案内だけを許し、発見を確定しない。unavailable は音・触覚・画面方向案内と発見をすべて停止し、状態説明と補正/練習への操作だけを残す。
...
- 独自の 8 の字案内は、精度が needsCalibration/unavailable の時だけ提示する。ゆっくり端末を 8 の字に動かし、その後静止して再判定する手順とする。
...
| ID | 合格条件 |
|---|---|
| A-SEN-01 | 有効方位精度の境界値と直前直後を注入し、`<=8° good`、`>8°..<=10° fair`、`>10°..<=25° needsCalibration`、無効値/`>25° unavailable` となる。unavailable、モーション利用不可、データ停止/陳腐化では全方向案内と発見を停止する。 |
| A-SEN-02 | 8 の字補正案内の前後値を再取得し、good/fairへ改善時だけ通常探索へ戻る。改善しない場合は粗い案内または安全な移動/練習を案内する。 |
| A-SEN-03 | 磁石付きケース、金属机、車内または鉄骨付近の少なくとも 2 条件で挙動を観察し、needsCalibration/unavailable時に警告し、制限説明へ到達できる。自動検出できなかった干渉も試験記録へ残す。 |
| A-SEN-04 | 角距離を 90°から 0°へ単調に減らす合成入力で、音の明瞭さ、触覚間隔、画面収束度が逆行しない。 |
| A-SEN-05 | 許容角が有効方位精度に応じて3〜6°となる境界を検証する。good/fair、ライブ高度2°以上、静止、データ新鮮の全条件下でも0.8秒未満の通過では発見せず、許容角内で0.8秒連続した場合だけ一度発見する。needsCalibration/unavailableでは一致しても発見しない。 |
...
| A-FIELD-05 | good/fairだけで発見を確定する。needsCalibrationでは粗い案内と補正へ移れ、unavailableでは全方向案内を停止して補正/練習へ移れる。 |
